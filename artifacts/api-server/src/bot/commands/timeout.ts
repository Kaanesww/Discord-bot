import {
  ChatInputCommandInteraction, GuildMember, PermissionFlagsBits, SlashCommandBuilder, EmbedBuilder,
} from "discord.js";
...
export const data = new SlashCommandBuilder()
  ...
  .addIntegerOption((o) => o.setName("dakika").setDescription("Süre (dakika, maks 40320 = 28 gün)").setMinValue(1).setMaxValue(40320).setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (!member.moderatable && !isOwner(interaction.user.id)) {
    await interaction.editReply("❌ Bu kullanıcıyı susturamıyorum. Rolü botunkinden yüksek olabilir."); return;
  }

  await member.timeout(dakika * 60 * 1000, `${interaction.user.tag}: ${sebep}`);
  await logAction({ guildId: interaction.guildId!, userId: targetUser.id, moderatorId: interaction.user.id, action: "timeout", reason: sebep, duration: dakika });

  const sureStr = dakika < 60 ? `${dakika} dakika` : dakika < 1440 ? `${Math.round(dakika / 60)} saat` : `${Math.round(dakika / 1440)} gün`;
  const embed = new EmbedBuilder().setColor(0x5865f2).setTitle("🔇 Kullanıcı Susturuldu")
