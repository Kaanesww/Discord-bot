...
export const data = new SlashCommandBuilder()
  ...
  .addIntegerOption((o) =>
    o.setName("adet").setDescription("Silinecek mesaj sayısı (1-100)").setMinValue(1).setMaxValue(100).setRequired(true),
  )
  .addUserOption((o) =>
    o.setName("kullanici").setDescription("Sadece bu kullanıcının mesajlarını sil").setRequired(false),
  )
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (toDelete.size === 0) {
    await interaction.editReply("❌ Silinecek mesaj bulunamadı.");
    return;
  }
  ...
  await interaction.editReply(
    `🗑️ **${deleted.size}** mesaj silindi${filterUser ? ` (<@${filterUser.id}> kullanıcısına ait)` : ""}.`,
  );
}
