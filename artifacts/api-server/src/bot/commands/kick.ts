...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (!member.kickable) { await interaction.reply({ content: "❌ Bu kullanıcıyı atamıyorum.", ephemeral: true }); return; }
  ...
  collector.on("collect", async (i) => {
    if (i.customId === "kick_cancel") {
      await i.update({ embeds: [new EmbedBuilder().setColor(0x72767d).setTitle("❌ İptal Edildi").setDescription("Kick işlemi iptal edildi.")], components: [] });
      return;
    }
    await member.kick(`${interaction.user.tag}: ${sebep}`);
