...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (member && !member.bannable && !isOwner(interaction.user.id)) {
    await interaction.reply({ content: "❌ Bu kullanıcıyı yasaklayamıyorum.", ephemeral: true }); return;
  }
  ...
  const confirmEmbed = new EmbedBuilder()
    .setColor(0xeb459e).setTitle("⚠️ Ban Onayı")
  ...
  collector.on("collect", async (i) => {
    if (i.customId === "ban_cancel") {
      await i.update({ embeds: [new EmbedBuilder().setColor(0x72767d).setTitle("❌ İptal Edildi").setDescription("Ban işlemi iptal edildi.")], components: [] }); return;
    }
    ...
    const embed = new EmbedBuilder().setColor(0xeb459e).setTitle("🔨 Kullanıcı Yasaklandı")
      .setThumbnail(targetUser.displayAvatarURL())
      .addFields({ name: "Kullanıcı", value: `${targetUser.tag} (<@${targetUser.id}>)`, inline: true }, { name: "Moderatör", value: `<@${interaction.user.id}>`, inline: true }, { name: "Sebep", value: sebep })
      .setTimestamp();
