...
function slotEmbed(r1: string, r2: string, r3: string, label: string, color: number) {
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (bal.coins < bet) {
    await interaction.editReply({
      embeds: [new EmbedBuilder().setColor(0xed4245).setTitle("❌ Yetersiz Bakiye").setDescription(`Bakiyen: **${bal.coins.toLocaleString("tr-TR")} ⬤V** · Bahis: **${bet.toLocaleString("tr-TR")} ⬤V**`)],
    });
    return;
  }
  ...
  let newBal: number;
  if (multiplier === 0) { newBal = await takeCoins(interaction.user.id, bet); }
  else if (diff > 0)   { newBal = await addCoins(interaction.user.id, diff); }
